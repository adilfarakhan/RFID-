...
<div class="main">
  ...
  <table>
    ...
    <?php
    $no = 1;
    $res = $koneksi->query("SELECT * FROM users ORDER BY id DESC");
    while ($row = $res->fetch_assoc()) {
      echo "<tr>
              ...
              <td>
                <a href='jadwal.php?id_guru={$row['id']}' style='text-decoration:none;'>
                  <button type=\"button\" class=\"jadwal-btn\" style=\"background:linear-gradient(90deg,#43e97b 0%,#38f9d7 100%);color:#fff;border:none;border-radius:6px;padding:7px 16px;font-weight:bold;cursor:pointer;transition:background 0.18s, color 0.18s, transform 0.18s;box-shadow:0 2px 8px rgba(44,62,80,0.08);font-size:0.98rem;\">
                    Jadwal
                  </button>
                </a>
              </td>
...
<div id="editPopup">
  <div class="popup-content">
    ...
    <form method="post" id="editForm">
      ...
      <div>
        <label>Jabatan</label>
        <input type="text" name="edit_jabatan" id="edit_jabatan" required>
      </div>
      <button type="submit" name="simpan">Simpan</button>
      <button type="button" class="cancel-btn" onclick="closePopup()">Batal</button>
      <button type="button" class="cancel-btn" style="background:#e53935;margin-left:8px;" onclick="hapusGuru()">Hapus</button>
    </form>
...
<!-- Pop Up Jadwal -->
<div id="jadwalPopup" style="display:none;position:fixed;top:0;left:0;width:100vw;height:100vh;background:rgba(0,0,0,0.4);z-index:9999;align-items:center;justify-content:center;">
  <div class="popup-content" style="background:#fff;padding:30px 24px 18px 24px;border-radius:14px;min-width:320px;max-width:95vw;position:relative;box-shadow:0 8px 32px rgba(44,62,80,0.13);">
    <h3 id="jadwalPopupTitle" style="color:#219a6f;margin-bottom:18px;text-align:center;">Jadwal Guru</h3>
    <div id="jadwalPopupBody" style="text-align:center;color:#888;">
    </div>
  </div>
</div>

<script>
  // Tampilkan popup dan isi data edit
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_id']) && isset($_POST['simpan'])) {
  ...
  $jabatan = $koneksi->real_escape_string($_POST['edit_jabatan']);
  $koneksi->query("UPDATE users SET nama='$nama', jabatan='$jabatan' WHERE id=$id");
